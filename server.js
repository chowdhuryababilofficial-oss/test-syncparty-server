...
const httpServer = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === "/api/scrapbook/resolve" && req.method === "POST") {
      ...
      const provisional = b.provisional && typeof b.provisional === "object" ? b.provisional : {};
      if (!process.env.TMDB_API_TOKEN) { json(res,503,{ok:false,error:"Metadata resolver is not configured."}); return; }
      const resolved = await metadataResolver.resolve({ meta, provisional });
      json(res,200,{ok:true,metadata:resolved});
      return;
    }
